...
The processed data is stored as `DataAssay` objects within the `scTypeEval`
...
sceval <- Run.HVG(
  scTypeEval = sceval,
  var.method = "basic",        # Method for variance modeling
...
sceval <- add.GeneList(
  scTypeEval = sceval,
  gene.list = list("TCR_signaling" = c("CD3D", "CD3E", "CD3G",
                                       "CD247", "LCK", "ZAP70")
                   )
)
