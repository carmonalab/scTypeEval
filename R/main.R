...
#' For each selected representation, one or more consistency metrics are
#' calculated per cell type and returned in a tidy \code{data.frame}.
#'
...
#'        in \code{scTypeEval@dissimilarity}. This argument is only used by
...
#'          \item \code{"MetaNeighbor_Supervised"} – Computes supervised
#'          cross-dataset replicability scores per cell type using
#'          \code{MetaNeighbor::MetaNeighbor}.
#'          \item \code{"nsa_silhouette"} – Silhouette score computed in
#'          single-cell embedding space (not sample-aware).
#'          \item \code{"nsa_cLISI"} – Cell-type cLISI consistency score
#'          computed in single-cell embedding space (not sample-aware).
#'          \item \code{"nsa_ROGUE"} – ROGUE purity score computed from
#'          single-cell counts and active labels (not sample-aware).
#'        }
#'        Default: dissimilarity-based metrics.
...
#' \code{\link{run_dissimilarity}} for dissimilarity-based metrics and additionally
#' supports non-sample-aware single-cell metrics and supervised MetaNeighbor.
#' All selected metrics are returned in a common long-format data frame,
#' allowing downstream comparison across cell types, metrics, and
...
get_consistency <- function(scTypeEval,
   ...
   dissimilarity_metrics <- c("silhouette",
                              "2label_silhouette",
                              "NeighborhoodPurity",
                              "ward_PropMatch",
                              "Orbital_medoid",
                              "Average_similarity")
   metric_dispatch <- c(dissimilarity_metrics,
                        "MetaNeighbor_Supervised",
                        "nsa_silhouette",
                        "nsa_cLISI",
                        "nsa_ROGUE")

   invalid_metrics <- setdiff(consistency_metric, metric_dispatch)
   if (length(invalid_metrics) > 0) {
      stop("Unsupported consistency metric(s): ",
           paste(invalid_metrics, collapse = ", "),
           call. = FALSE)
   }

   diss_metric <- intersect(consistency_metric, dissimilarity_metrics)
   other_metric <- setdiff(consistency_metric, dissimilarity_metrics)
   needs_diss_assay <- length(diss_metric) > 0 ||
      "MetaNeighbor_Supervised" %in% consistency_metric
   ...
   if (length(diss_metric) > 0) {
      ...
      consist <- dplyr::bind_rows(consist, dplyr::bind_rows(consist.list))
   }

   if (length(other_metric) > 0) {
      other.list <- lapply(other_metric,
                           function(metric) {
                              if(verbose){message("Computing ", metric, " ... \n")}
                              switch(metric,
                                     "MetaNeighbor_Supervised" = MN_supervised(scTypeEval = scTypeEval,
                                                                                verbose = verbose),
                                     "nsa_silhouette" = nsa_silhouette(scTypeEval = scTypeEval,
                                                                       slot = "single-cell"),
                                     "nsa_cLISI" = nsa_clisi(scTypeEval = scTypeEval,
                                                             slot = "single-cell"),
                                     "nsa_ROGUE" = nsa_rogue(scTypeEval = scTypeEval,
                                                             slot = "single-cell"),
                                     stop(metric, " is not a supported consistency method.", call. = FALSE)
                              )
                           })
      consist <- dplyr::bind_rows(consist, dplyr::bind_rows(other.list))
   }
