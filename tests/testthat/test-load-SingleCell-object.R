# Test suite for load_single_cell_object function
# 
# This file tests the load_single_cell_object function which handles
...
test_that("load_single_cell_object loads Seurat object with split=TRUE", {
  skip_if_not_installed("Seurat")
  
  filepath <- create_test_seurat_file("seurat_test.rds")
  
  result <- load_single_cell_object(filepath, split = TRUE)
...
test_that("load_single_cell_object loads Seurat object with split=FALSE", {
  skip_if_not_installed("Seurat")
  
  filepath <- create_test_seurat_file("seurat_test2.rds")
  
  result <- load_single_cell_object(filepath, split = FALSE)
...
test_that("load_single_cell_object loads SCE object with split=TRUE", {
  skip_if_not_installed("SingleCellExperiment")
  skip_if_not_installed("SummarizedExperiment")
...
test_that("load_single_cell_object loads SCE object with split=FALSE", {
  skip_if_not_installed("SingleCellExperiment")
  skip_if_not_installed("SummarizedExperiment")
...
test_that("load_single_cell_object errors when file does not exist", {
  nonexistent_file <- file.path(tempdir(), "nonexistent_file.rds")
  
  expect_error(
    load_single_cell_object(nonexistent_file, split = TRUE),
    "File does not exist"
  )
})

test_that("load_single_cell_object errors with unsupported .rds object type", {
  test_dir <- file.path(tempdir(), "scTypeEval_load_tests")
  dir.create(test_dir, recursive = TRUE, showWarnings = FALSE)
...
test_that("load_single_cell_object errors with unsupported file format", {
  test_dir <- file.path(tempdir(), "scTypeEval_load_tests")
  dir.create(test_dir, recursive = TRUE, showWarnings = FALSE)
...
test_that("load_single_cell_object accepts boolean split parameter", {
  skip_if_not_installed("Seurat")
  
  filepath <- create_test_seurat_file("seurat_test_bool.rds")
  
  # Test with TRUE
  result_true <- load_single_cell_object(filepath, split = TRUE)
  expect_type(result_true, "list")
  
  # Test with FALSE
  result_false <- load_single_cell_object(filepath, split = FALSE)
...
test_that("loaded Seurat data can create scTypeEval object", {
  ...
  obj <- create_scTypeEval(
    matrix = result$counts,
    metadata = result$metadata,
    active_ident = "cell_type"
  )
  obj <- set_active_ident(obj, ident = "sample")
...
test_that("loaded Seurat object directly can create scTypeEval object", {
  ...
  seurat_obj <- load_single_cell_object(filepath, split = FALSE)
...
test_that("loaded SCE object directly can create scTypeEval object", {
  ...
  sce_obj <- load_single_cell_object(filepath, split = FALSE)
